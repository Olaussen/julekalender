...
def part2():
    ...
    instructions = [[int(y) for y in x if y] for x in [re.split(r'[^0-9]', x) for x in [x.strip() for x in data[1]]]]
    for instruction in instructions:
        containers[instruction[1] - 1], containers[instruction[2] - 1] = containers[instruction[1] - 1][:-instruction[0]], containers[instruction[2] - 1] + containers[instruction[1] - 1][-instruction[0]:]
